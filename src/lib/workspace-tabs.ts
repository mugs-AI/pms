...
export const PROJECT_SECTIONS: ProjectSection[] = ["overview", "phases", "team", "activity", "budget", "quotation"];
...
export function getWorkspaceTabs(): WorkspaceTab[] { return tabs; }
export function getWorkspaceCapMessage(): string | null { return capMessage; }
...
export function openNewEnquiryWorkspace(): void {
  ...
  tabs = existing
    ...
    : [...tabs, { key: "new-enquiry", projectId: null, reference: "New Enquiry", title: "New Enquiry", section: "overview", lastUsed: nextUse(), dirty: false }];
  emit();
}
...
export function replaceNewEnquiryWithProject(input: Omit<WorkspaceTab, "key" | "lastUsed" | "dirty">): void {
  tabs = tabs.filter((tab) => tab.key !== "new-enquiry");
  openProjectWorkspace(input);
}

export function resetWorkspaceTabsForTests(): void { clearWorkspaceTabs(); }
