...
export type ActiveWorkspace =
  | { kind: "new" }
  | { kind: "project"; projectId: string; section: ProjectSection; reference?: string; title?: string };

export function WorkspaceTabs({ active }: { active?: ActiveWorkspace }) {
  const tabs = useWorkspaceTabs();
  const navigate = useNavigate();
  const refs = useRef<(HTMLAnchorElement | null)[]>([]);
  const activeKey = active?.kind === "new" ? "new-enquiry" : active?.kind === "project" ? `project:${active.projectId}` : null;
  ...
  }, [active?.kind, active?.kind === "project" ? active.projectId : "", active?.kind === "project" ? active.reference : "", active?.kind === "project" ? active.title : "", active?.kind === "project" ? active.section : ""]);
  ...
  const close = async (key: string, index: number) => {
    ...
    if (wasActive) {
      ...
      else if (next.projectId) await navigate({ to: "/projects/$projectId", params: { projectId: next.projectId }, search: { section: next.section } });
  ...
  return (
    <div className="border-t border-primary-foreground/10 bg-primary" aria-label="Open workspaces">
      <div className="overflow-x-auto px-4 sm:px-6 lg:px-8">
        <div role="tablist" aria-label="Open project workspaces" className="flex min-w-max items-stretch gap-1 py-1.5">
          {tabs.map((tab, index) => {
                ...
                {tab.projectId ? (
                  <Link
                    ...
                    onKeyDown={(event) => {
                      ...
                      event.preventDefault(); refs.current[target]?.focus();
                    }}
                  >{label}</Link>
                ) : (
                  <Link
      ...
      {capMessage ? <p role="status" className="px-4 pb-2 text-xs text-primary-foreground sm:px-6 lg:px-8">{capMessage}</p> : null}
    </div>
  );
}
